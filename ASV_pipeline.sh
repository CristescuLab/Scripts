...
# run first cutadapt
cutadapt -g "${1}" -G "${2}" -a "${3}" -A "${4}" -o ${5}/${6}.1.fastq.gz \
-p ${5}/${6}.2.fastq.gz -m 130 --match-read-wildcards -q 25 --trim-n \
-n 2 --untrimmed-output ${5}/untrimmed.${6}.fastq --untrimmed-paired-output \
${5}/untrimmed.paired.${6}.fastq ${7}_R1.fastq.gz \
${7}_R2.fastq.gz > ${5}/${6}.log1
# Merge reads
pear -f ${5}/${6}.1.fastq.gz -r ${5}/${6}.2.fastq.gz -o ${5}/${6}_pear -q 25 \
-t 100 -s 2 > ${5}/${6}_pear.log
# check if adapters still there
seqkit -j 8 locate -d -p "${1}" ${5}/${6}_pear.assembled.fastq > ${5}/${6}.fwd
seqkit -j 8 locate -d -p "${2}" ${5}/${6}_pear.assembled.fastq > ${5}/${6}.rev
# cut the adapter if they remain
if [[ $(wc -l <${5}/${6}.fwd) -ge 2 || $(wc -l <${5}/${6}.rev) -ge 2 ]]; then
cutadapt -a "$3" -m 100 -n 2 --too-short-output ${5}/${6}.short.fastq \
-o ${5}/${6}.3trimmed.fastq ${5}/${6}_pear.assembled.fastq > ${5}/${6}.log2
cutadapt -g "${1}" -n 2 -o ${5}/${6}.5trimmed.fastq \
${5}/${6}.3trimmed.fastq > ${outdir}/${6}.log3; else
cp ${5}/${6}_pear.assembled.fastq ${5}/${6}.3trimmed.fastq
fi
# dereplicate
usearch -fastx_uniques ${5}/${6}.3trimmed.fastq -fastaout \
${5}/${6}.trimmed.derep.fasta -sizeout
# get lenghth distributions
length_stats ${5}/${6}.trimmed.derep.fasta ${5}/${6}
# get stats for all the steps
seqkit stats ${5}/*${6}*.fa* > ${5}/${6}.stats
# run a fastqc
fastqc ${5}/${6}.3trimmed.fastq -o ${5}
}
...
# run QC in all pairs
export -f run_QCsingle_pair
parallel --will-cite --joblog ${outdir}/${prefix}_QC.log -j ${cpus} --wd . \
run_QCsingle_pair ${ADAPTER_FWD} ${ADAPTER_REV} ${Adapter2rc} ${Adapter1rc} \
${outdir} ${prefix} {} :::: ${file_list}

# Trim the reads to expected lenght
for i in ${outdir}/*3trimmed.fastq
do
    cat ${i} | seqkit -j ${cpus} seq -m ${min_len} \
    -M ${max_len} > $outdir/{i%%3trimmed.fastq}lengthfilter.fastq
done
...
${outdir}/allzotus_lengthfilter_4.txt -sizeout

# Rename to Zotu
sed -i "/>O/>Zo/" ${outdir}/all_lengthfilter_zotus_4.fasta

# reannotate each sample
parallel
for i in ${outdir}/*lengthfilter.fastq
do
    usearch -fastx_relabel ${i} -prefix ${i%%.lengthfilter.fastq} \
